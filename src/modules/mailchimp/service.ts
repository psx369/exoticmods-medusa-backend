...
// The docs guide imports these two from /utils, but they are types and live in
// /types -- importing them from /utils fails to compile.
import {
  ...
  async sendNewsletterSignup(
    ...
    try {
      ...
      throw new MedusaError(
        ...
        `Failed to send newsletter signup: ${error.response?.text || error}`
      )
    }
  }
